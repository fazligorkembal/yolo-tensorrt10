...
void ModelDetectionScratch::serialize(std::string &wts_name, std::string &engine_path, std::string &type, float &gd, float &gw, int &max_channels)
{
    ...
    serialized_engine = buildEngineYolo11Det(builder, config, DataType::kFLOAT, options_.model_path, gd, gw, max_channels, type);
    ...
    std::cout << "Serialized engine to " << engine_path << std::endl;
    p.close();
...
{
    ASSERT(task_type == TaskType::detection, "Task type must be detection for ModelDetectionScratch");
    ASSERT(engine->getNbBindings() == 2, "ModelDetectionScratch must have exactly 2 bindings");
    
    const int inputIndex = engine->getBindingIndex(kInputTensorName);
    const int outputIndex = engine->getBindingIndex(kOutputTensorName);

    ASSERT(inputIndex == 0, "Input tensor must be at index 0");
    ASSERT(outputIndex == 1, "Output tensor must be at index 1");
